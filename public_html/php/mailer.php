...
// jquery validate will ajax the form, submit the recaptcha verification, before try catch, we'll get the google response
...
try {
	...
	// if google recaptcha passed front end, now it has to pass the back end and if so, throw exception
	if (!$resp->isSuccess()) {
	...
	// these corespond to the form names with sanitization; you'll see this is in html name=" - name -"
	...
	// found in mail recipeients in configuration mail file
	 ...
	 **/
	// we're encoding in plain text, refer to documentation for different type of text.
	...
	// we echo div with success messages
